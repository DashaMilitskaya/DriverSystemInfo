...
--*/
{
	...
	switch (IoControlCode)
	{
		...
		case GETIDTABLE:
			...
			WdfRequestCompleteWithInformation(Request, status, Resquestlength);
			break;
		case GETSYSCALL64TRACETABLE:
			status = PrintHookedItem(Request, InputBufferLength, OutputBufferLength, &Resquestlength);
			...
			WdfRequestCompleteWithInformation(Request, status, Resquestlength);
			break;
		case SETSYSCALL64HOOK:
			hookSystemCallx64();
			WdfRequestComplete(Request, STATUS_SUCCESS);
			break;
		case UNSETSYSCALL64HOOK:
			UnHookSystemCallx64();
			WdfRequestComplete(Request, STATUS_SUCCESS);
			break;
		default:
